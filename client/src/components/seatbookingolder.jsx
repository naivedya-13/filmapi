...
const SeatBooking = () => {
  ...
  try {
    ...
    const relevantBookings = validBookings.filter(b => 
      b.filmId === filmId && 
      b.showTime === showTime
    );
    const booked = [];
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      if (conflictingBookings.length > 0) {
        throw new Error("Some seats were just booked by another user. Please refresh and try again.");
      }
  ...
  const handleShowTimeSelect = (timeValue, display) => {
    setShowTime(timeValue);
    setShowTimeDisplay(display);
    // Clear selected seats when changing show time
    setSelectedSeats([]);
  };

  // No movie selected view
  if (!location.state) {
    return (
      <div className="p-8 text-center">
        <h2 className="text-2xl font-bold text-red-500">No movie selected</h2>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              ...
              <button
                ...
                className={`px-6 py-2 rounded-md text-white mt-4 md:mt-0
                  ${
                    isSubmitting || selectedSeats.length === 0 || !showTime || isLoading
